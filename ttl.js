...
let ttlhd = $.isNode() ? (process.env.ttlToken ? process.env.ttlToken : "") : ($.getdata('ttlToken') ? $.getdata('ttlToken') : "")
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        if (!$.isNode()) {
            ttlhdArr.push($.getdata('ttlToken'))
            let ttlcount = ($.getval('ttlcount') || '1');
            for (let i = 2; i <= ttlcount; i++) {
                ttlhdArr.push($.getdata(`ttlToken${i}`))
            }
            ...
            if (process.env.ttlToken && process.env.ttlToken.indexOf('@') > -1) {
                ttlhdArr = process.env.ttlToken.split('@');
                console.log(`您选择的是用"@"隔开\n`)
            } else {
                ttlhds = [process.env.ttlToken]
            }
            ;
...
function ttlck() {
    if ($request.url.indexOf("user") > -1) {
        ...
        if (ttlhd) $.setdata(ttlhd, `ttlToken${status}`)
